...
$(document).on('DOMContentLoaded', () => {
  $('#assigned_care_requests_list td').each((index, element) => $(element).css('width', $(element).width()));

  $('#assigned_care_requests_list').sortable({
    placeholder: 'ui-state-highlight',
    axis: 'y',
    // items: '.patient-data',
    items: '.care-request-data',
    cursor: 'move',
    sort(e, ui) {
    ...
    update(e, ui) {
      ...
      return $.ajax({
        type: 'PATCH',
        url: '/care_request_lists/reorder_care_request_list',
        dataType: 'text',
