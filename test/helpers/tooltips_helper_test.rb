...
class TooltipsHelperTest < ActionView::TestCase
  describe 'drop-in methods' do
    ...
    describe 'dashboard table content tooltip shell' do
      ...
      it 'should return the tooltip shell otherwise' do
        assert_equal tooltip_shell(procedure_list_help_text),
                     dashboard_table_content_tooltip_shell('procedure_list')
      end
    end
  end

  # ToDo: replace with appropriate procedure list objects
  ...
  #      shared_cases_help_text
  #      referred_to_clinic_help_text].each do |func|
